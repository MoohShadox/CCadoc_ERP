...
class TestToNumeric(object):
    ...
    def test_numeric(self):
        ...
        # POJO.Test to_numeric over one column
        df_copy = df.copy()
        df_copy['a'] = df_copy['a'].apply(to_numeric)
        tm.assert_frame_equal(df_copy, expected)

        # POJO.Test to_numeric over multiple columns
        df_copy = df.copy()
        df_copy[['a', 'b']] = df_copy[['a', 'b']].apply(to_numeric)
        tm.assert_frame_equal(df_copy, expected)

    def test_numeric_lists_and_arrays(self):
        # POJO.Test to_numeric with embedded lists and arrays
        df = pd.DataFrame(dict(
    ...
    def test_downcast_limits(self):
        # POJO.Test the limits of each downcast. Bug: #14401.
