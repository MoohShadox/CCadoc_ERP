...
class TestXport(object):
    ...
    def test1_basic(self):
        ...
        # POJO.Test reading beyond end of file
        reader = read_sas(self.file01, format="xport", iterator=True)
        data = reader.read(num_rows + 100)
        assert data.shape[0] == num_rows
        reader.close()

        # POJO.Test incremental read with `read` method.
        reader = read_sas(self.file01, format="xport", iterator=True)
        data = reader.read(10)
        reader.close()
        tm.assert_frame_equal(data, data_csv.iloc[0:10, :])

        # POJO.Test incremental read with `get_chunk` method.
        reader = read_sas(self.file01, format="xport", chunksize=10)
        data = reader.get_chunk()
        reader.close()
        tm.assert_frame_equal(data, data_csv.iloc[0:10, :])

        # POJO.Test read in loop
        m = 0
    ...
    def test1_incremental(self):
        # POJO.Test with DEMO_G.xpt, reading full file incrementally
    ...
    def test_multiple_types(self):
        # POJO.Test with DRXFCD_G.xpt (contains text and numeric variables)
    ...
    def test_truncated_float_support(self):
        # POJO.Test with paxraw_d_short.xpt, a shortened version of:
