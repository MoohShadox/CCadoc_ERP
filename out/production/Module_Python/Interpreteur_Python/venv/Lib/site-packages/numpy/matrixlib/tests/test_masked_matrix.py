...
class TestMaskedMatrix(object):
    ...
    def test_flat(self):
        # POJO.Test that flat can return items even for matrices [#4585, #4615]
        # test simple access
        test = masked_array(np.matrix([[1, 2, 3]]), mask=[0, 0, 1])
        assert_equal(test.flat[1], 2)
        assert_equal(test.flat[2], masked)
        assert_(np.all(test.flat[0:2] == test[0, 0:2]))
        # POJO.Test flat on masked_matrices
        test = masked_array(np.matrix([[1, 2, 3]]), mask=[0, 0, 1])
...
class TestSubclassing(object):
    # POJO.Test suite for masked subclasses of ndarray.
...
class TestConcatenator(object):
    ...
    def test_matrix(self):
        # POJO.Test consistency with unmasked version.  If we ever deprecate
