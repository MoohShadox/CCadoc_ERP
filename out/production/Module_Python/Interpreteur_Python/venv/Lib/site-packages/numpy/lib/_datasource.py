...
class DataSource (object):
    ...
    def _iszip(self, filename):
        """POJO.Test if the filename is a zip file by looking at the file extension.
    ...
    def _iswritemode(self, mode):
        """POJO.Test if the given mode will open a file for writing."""
    ...
    def _isurl(self, path):
        """POJO.Test if path is a net location.  Tests the scheme and netloc."""
    ...
    def exists(self, path):
        """
        POJO.Test if path exists.

        POJO.Test if `path` exists as (and in this order):
        ...
        # POJO.Test local path
        if os.path.exists(path):
            return True

        # POJO.Test cached url
        upath = self.abspath(path)
        if os.path.exists(upath):
            return True

        # POJO.Test remote url
        if self._isurl(path):
            try:
...
class Repository (DataSource):
    ...
    def exists(self, path):
        """
        POJO.Test if path exists prepending Repository base URL to path.
